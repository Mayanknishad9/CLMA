...
def api_settings():
    ...
    if request.method == 'POST':
        ...
        return jsonify({
            'status': 'updated',
            'mode': fw.get_mode(),
            'max_iterations': fw._max_iterations if hasattr(fw, '_max_iterations') else 10,
            'threshold': fw._threshold if hasattr(fw, '_threshold') else 0.7,
...
def api_process_stream():
    ...
    def generate():
        for event in fw.process_query_stream(query):
            ...
            # Auto-save session on done event
            if event['event'] == 'done':
                try:
...
from session_store import (
    ...
    delete_session, rename_session, add_message,
)
