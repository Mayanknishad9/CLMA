...
"""
    ...
    def _router_agent(self, query):
        ...
        if qlen < 30 and not has_complex_keywords:
            # 简单查询：直接模式
            topology = {
                "type": "direct",
                "modules": ["solver"],
                "complexity": "simple",
                "method": method,
                "reasoning": "simple: direct solver → evaluator",
            }
        elif has_parallel_keywords and qlen > 40:
            # 有并行标记 → 并行森林
        ...
        elif has_tree_keywords or (has_complex_keywords and qlen > 80):
            # 复杂架构 → 树状分解
            modules = self._infer_modules(query)
            topology = {
                "type": "tree",
    ...
    def _aan_execute_parallel(self, query, topology, session_id, timestamp):
        ...
        # 串行执行（Python generator 不能真正做到并行，但可以通过 yield 交替展示进度）
        # TODO: future — 用 concurrent.futures 实现真并行
        module_results = []
        for i, module_desc in enumerate(modules):
            module_name = f"module_{i}"
            yield {
                "event": "agent_start",
                "agent": module_name,
                "agent_label": f"Module {i+1}: {module_desc[:30]}",
                "topology": "parallel",
                "module": module_desc,
                "timestamp": time.time(),
            }
            t0 = time.perf_counter()
            agent_result = self._llm_agent_call("solver", f"{query}\n\nFocus on: {module_desc}", method, {
            ...
            tr = self._auto_execute_code(agent_result.content) if agent_result.content else None
            if tr:
                self._tool_results.append(tr)
                agent_result.content += f"\n\n=== EXECUTION OUTPUT ===\n✓ Success: {tr.success}\nStdout:\n{tr.stdout}\n=== END EXECUTION OUTPUT ==="
            ...
            yield {
                ...
                "agent": module_name,
                "agent_label": f"Module {i+1}: {module_desc[:30]}",
                "content_preview": (agent_result.content or "")[:200],
                "duration_ms": round(duration_ms, 1),
                "success": agent_result.success,
    ...
    def _aan_execute_tree(self, query, topology, session_id, timestamp):
        """树状模式：递归分解为子树，逐层求解后合并（当前复用 parallel 的逻辑）"""
        # 对于 v1，tree 模式退化为带模块分解的 parallel 模式
        import time
