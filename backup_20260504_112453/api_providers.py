...
Supports OpenAI, Anthropic Claude, DeepSeek, Google Gemini, Kimi, MiniMax, GLM,
...
PROVIDER_MODELS = {
    ...
    "minimax": [
        "minimax-text-01", "minimax-text-01-pro", "minimax-text-01-som",
        "abab6.5s-chat", "abab6.5g-chat",
        "minimax-m2",
    ],
    "glm": [
        "glm-4-plus", "glm-4-0520", "glm-4-air", "glm-4-airx",
        "glm-4-flash", "glm-4-flashx", "glm-4.5", "glm-4.6",
...
class LLMProvider:
    ...
    def __init__(self, config: dict, default_key: str = ""):
        self._default_key = default_key or self.__class__.__name__.replace("Provider", "").lower()
    ...
    def default_model(cls) -> str:
        # Map subclass names to PROVIDER_DEFAULTS keys
        name = cls.__name__.lower().replace("provider", "")
        lookup = {
            "_kimi": "kimi",
            "_minimax": "minimax",
            "_glm": "glm",
        ...
        return PROVIDER_DEFAULTS.get(key, {}).get("model", "")
    ...
    def default_base_url(cls) -> str:
        ...
        key = lookup.get(name, name)
        return PROVIDER_DEFAULTS.get(key, {}).get("base_url", "")
...
# Chinese OpenAI-compatible providers — reuse OpenAIProvider
# Register with a proper default_key so defaults resolve correctly
class _KimiProvider(OpenAIProvider):
    pass

class _MiniMaxProvider(OpenAIProvider):
    pass

class _GLMProvider(OpenAIProvider):
    pass

register_provider("kimi")(_KimiProvider)
register_provider("minimax")(_MiniMaxProvider)
register_provider("glm")(_GLMProvider)
